...
@bot.tree.command(name="profile", description="Show a member's avatar")
@app_commands.describe(member="Select a member (leave empty for yourself)")
...
@bot.tree.command(name="me", description="Show a member's profile")
@app_commands.describe(member="Select a member (leave empty for yourself)")
async def profile_slash(interaction: discord.Interaction, member: discord.Member = None):
    member = member or interaction.user
    cur.execute("SELECT messages FROM xp WHERE guild_id=? AND user_id=?", (str(interaction.guild.id), str(member.id)))
    row = cur.fetchone()
